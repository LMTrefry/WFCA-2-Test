...
public class Training : MonoBehaviour{
	public int gridsize = 1; //Size of frame (spreads out waypoints)
	public int width = 12;
	public int depth = 12;
	public UnityEngine.Object[] tiles = new UnityEngine.Object[0]; //Array of prefabs (input)
	...
	public void RecordNeighbors() { //Records neighbors of input, puts it in XML document.
		Compile();
	...
	public string AssetPath(UnityEngine.Object o){ //Places scripts in appropriate places on download.
		#if UNITY_EDITOR
	...
	public string NeighborXML(){ //Creates the NXL document.
	...
	public bool hasWhitespace(){ //Checks if there is white space.
		byte ws = (byte)0;
	...
	public void Compile() { //Captures the inputs.
		str_tile = new Dictionary<string, byte>(); //
		sample = new byte[width, depth]; 
		int cnt = this.transform.childCount; //# of children. <---------------------------------------------------------
		tiles = new UnityEngine.Object[500]; //Array of tiles.
		RS = new int[1000];
		tiles[0] = null; //First elements of tiles = null.
		RS[0] = 0;
		for (int i = 0; i < cnt; i++){ //Records the positions of the inputs.
			GameObject tile = this.transform.GetChild(i).gameObject;
			Vector3 tilepos = tile.transform.localPosition;
			
			if ((tilepos.x > -0.55f) && (tilepos.x <= width*gridsize-0.55f) && //Puts the blue ball in the input.
	...
	void OnDrawGizmos(){ //Draws wireframe
