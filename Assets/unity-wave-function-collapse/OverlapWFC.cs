...
public class OverlapWFC : MonoBehaviour{
	...
	static GameObject CreatePrefab(UnityEngine.Object fab, Vector3 pos, Quaternion rot) { //Stores prefabs rotations and position
		#if UNITY_EDITOR
	...
	public void Clear(){ //Clears output at start
		if (group != null){
	...
	void Awake(){} //Called when script instance is being loaded, sets GameObjects to active.

	void Start(){ //On start calls generate
		Generate();
	}

	void Update(){ //Every epdate, if incremental true, call Run
		if (incremental){
			Run();
		}
	}

	public void Generate() { //Generates an output
		if (training == null){Debug.Log("Can't Generate: no designated Training component");} //No input, generate error
		if (IsPrefabRef(training.gameObject)){
			GameObject o = CreatePrefab(training.gameObject, new Vector3(0,99999f,0f), Quaternion.identity); //Creates o
			training = o.GetComponent<Training>(); //Gets training componenets
		}
		if (training.sample == null){ //Compile if it isn't already.
			training.Compile();
		}
		if (output == null){
		...
		for (int i = 0; i < output.transform.childCount; i++){ //Gets children.
		...
		group.rotation = output.transform.rotation; //Determines position
	...
	void OnDrawGizmos(){ //Draws the output wireframe
	...
	public void Run(){ //On run, call draw.
	...
	public void Draw(){ //Places the generated input.
		if (output == null){return;}
		if (group == null){return;}
        undrawn = false;
		try{ //As long as index not out of range, do this.
			for (int y = 0; y < depth; y++){ //Traverse depth.
				for (int x = 0; x < width; x++){ //Traverse width.
